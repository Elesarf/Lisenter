    ...
    switch ( _dispencer[numPlace].state ) {
        case S_START: {
            for ( int i = 1; i < _buffer.length(); ++i ) {
                if ( _buffer.at(i - 1) == _jpg[0] && _buffer.at(i) == _jpg[1] ) {
                    if ( i > 6 ) {
                        _dispencer[numPlace].place	= _buffer.at(i - 7);
        ...
        case S_READ: {
            ...
            if ( !indx ) {
                ...
                emit imgComplete(_dispencer[numPlace].frame,  _dispencer[numPlace].place,  _dispencer[numPlace].cam,  _dispencer[numPlace].size);
                _dispencer[numPlace].state = S_START;
